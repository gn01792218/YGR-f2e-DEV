      ...
      <div>
        ...
        <label for="plusMoneyInput">
          ...
          <input class="primary-input w-[100px] md:w-[150px] mr-5" type="text" v-model="plusMoneyInput"
            @keyup.enter="plusMoney(plusMoneyInput)" @focus="setNumberNull">
        </label>
      </div>
      <div class="flex flex-col">
        <button class="primary-btn mb-5 text-sm">清空帳號金額</button>
        <button class="primary-btn text-sm" @click="plusMoney(plusMoneyInput)">確認增加</button>
      </div>
    </div>
...
const { userName, plusMoneyInput, setNumberNull, plusMoney } = useUser()
</script>